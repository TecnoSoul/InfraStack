...
#
# This library provides: CSV-based station/container inventory tracking
#=============================================================================
...
# Inventory file location (can be overridden via environment or config)
# Default uses infrastack path, but RADIO_INVENTORY_FILE provides backwards compatibility
INVENTORY_FILE="${RADIO_INVENTORY_FILE:-${INVENTORY_FILE:-/etc/infrastack/inventory/stations.csv}}"
INVENTORY_BACKUP_DIR="${INVENTORY_BACKUP_DIR:-/etc/infrastack/inventory/backups}"
...
# Parameters:
#   $1 - ctid
#   $2 - type (azuracast/libretime/icecast/other)
#   $3 - hostname
#   $4 - ip
#   $5 - description
# Returns: 0 on success, 1 on failure
...
list_all_stations() {
    ...
    log_info "Station Inventory ($count stations):"
    echo ""
