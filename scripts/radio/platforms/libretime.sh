#!/bin/bash
#=============================================================================
# InfraStack Radio Module - LibreTime Platform Deployment
...
install_libretime() {
    ...
    if ! pct exec "$ctid" -- bash -c '
        ...
        # Configure docker-compose.yml to use mounted HDD storage
        ...
        # Add restart policy
        sed -i "/^services:/a\\  # Auto-restart configuration" docker-compose.yml
...
display_libretime_success() {
    ...
    echo "  Update:         infrastack radio update --ctid $ctid"
    echo "  Backup:         infrastack radio backup --ctid $ctid"
    echo "  Logs:           infrastack radio logs --ctid $ctid"
    echo "  Status:         infrastack radio status --ctid $ctid"
    echo ""
    echo "Documentation:"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    show_help() {
        cat << EOF
InfraStack Radio Module - LibreTime Deployment Script
