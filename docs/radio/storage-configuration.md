# Radio Module Storage Configuration

## Overview

The InfraStack Radio Module uses a **two-tier storage architecture** for optimal performance and cost efficiency:
...
When you deploy a radio platform using InfraStack, the following happens:
...
**Symptoms:**
- Upload media files via web interface
...
**Cause:**
This can happen if:
1. You deployed before the storage fix was implemented
2. You manually installed without using InfraStack scripts
3. The deployment script failed during the storage configuration step

**Solution:**
Contact support or manually migrate data (see Manual Storage Configuration below).
...
```bash
#!/bin/bash
# Check all radio platform storage usage

echo "Radio Platform Storage Report"
echo "=============================="
echo ""

zfs list -o name,used,avail,refer,quota | grep -E "(azuracast|libretime)-media" | while read line; do
    echo "$line"
done
```
...
InfraStack automatically sets optimal ZFS compression:
```bash
