...
Complete guide for deploying and managing LibreTime radio automation software with InfraStack.
...
InfraStack deploys LibreTime 4.5.0 using Docker containers in optimized LXC containers.
...
sudo infrastack radio deploy libretime \
  -i 202 \
  -n fm-rock \
  -c 4 \
  -m 8192 \
...
Or use InfraStack:
```bash
sudo infrastack radio backup --ctid 201 --type full
```
...
Or use InfraStack:
```bash
sudo infrastack radio update --ctid 201
```
...
- GitHub: https://github.com/TecnoSoul/InfraStack
- Issues: Report bugs specific to InfraStack deployment
...
2. **Deploy new InfraStack container**
   ```bash
   sudo infrastack radio deploy libretime -i 201 -n station1
   ```
